...
AABB Transform::operator()(const AABB &b) const {
    ...
    AABB ret(        M(Point(b.pMin.x, b.pMin.y, b.pMin.z)));
...
/*
//Arvo, J. 1990. Transforming axis-aligned bouding boxes

	Vector T(m.m[0][3], m.m[1][3], m.m[2][3]);

	AABB ret;
	for(int i = 0; i < 3; i++) {
		ret.pMin[i] = ret.pMax[i] = T[i];
		for(int j = 0; j < 3; j++) {
			float x = m.m[i][j]*b.pMin[j];
			float y = m.m[i][j]*b.pMax[j];
